...
public class FileConfig {
    ...
    public FileConfiguration get(String paramString) {
        if (isFileLoaded(paramString)) {
            return configs.get(paramString);
        }
        return null;
    }
    ...
    public boolean update(String paramString1, String paramString2, Object paramObject) {
        if ((isFileLoaded(paramString1)) &&
                (!configs.get(paramString1).contains(paramString2))) {
            configs.get(paramString1).set(paramString2, paramObject);
            return true;
        }
        return false;
    }
    ...
    public void addComment(String paramString1, String paramString2, String[] paramArrayOfString) {
        if (isFileLoaded(paramString1)) {
            ...
            for (int i = 0; i < j; i++) {
                ...
                if (!configs.get(paramString1).contains(paramString2)) {
                    configs.get(paramString1).set("_COMMENT_" + paramArrayOfString.length, " " + str);
                }
            }
        }
    }
    ...
    public void remove(String paramString1, String paramString2) {
        if (isFileLoaded(paramString1)) {
            configs.get(paramString1).set(paramString2, null);
        }
    }

    public boolean contains(String paramString1, String paramString2) {
        if (isFileLoaded(paramString1)) {
            return configs.get(paramString1).contains(paramString2);
        }
        return false;
    }
    ...
    public void reload(String paramString) {
        ...
        if (isFileLoaded(paramString)) {
            try {
                configs.get(paramString).load(localFile);
            } catch (Exception localException) {
                localException.printStackTrace();
            }
        }
    ...
    public void save(String paramString) {
        ...
        if (isFileLoaded(paramString)) {
            try {
                configs.get(paramString).save(localFile);
            } catch (Exception localException) {
                localException.printStackTrace();
            }
        }
